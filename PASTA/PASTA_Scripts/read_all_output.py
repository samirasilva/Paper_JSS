...
def compute_total_time(number_of_ex,file):
    ...
    for s in range(0,number_of_ex):
        nome_do_arquivo = './output_'+str(s)+'/time.txt'
...
def compute_average_vector(vectors):
    ...
    # Verificar se os vectors foram lidos corretamente
    if vectors:
        # Calculate the average of each position among all vectors
        media_vectors = [round(sum(coluna) / len(vectors),2) for coluna in zip(*vectors)]

        #print("Vetor de averages:", media_vectors)
        return media_vectors
        

def read_vectors(nome_arquivo):
    ...
    #print("Vetor:", current_vector)
    return current_vector
    
def compute_average_cov(number_of_patients,number_of_ex,file):

    averages=[]
    for s in range(0,number_of_ex):
        vectors = []
	for j in range(0,number_of_patients):
		filename = './output_'+str(s)+'/cov_'+str(j)+'.txt'
		vectors.append(read_vectors(filename))
	average_vector=compute_average_vector(vectors)
	averages.append(average_vector)

    m=compute_average_vector(averages)
    file.write("\nAverage Coverage Vector for all executions:\n")
    file.write("Oxi\tEcg\tTemp\tAbps\tAbpd\tGluc\tAvg|Std\n")
    file.write(str(float("{:.2f}".format(m[0])))+"\t"+str(float("{:.2f}".format(m[1])))+ "\t"+str(float("{:.2f}".format(m[2])))+"\t"+str(float("{:.2f}".format(m[3])))+ "\t"+str(float("{:.2f}".format(m[4])))+"\t"+str(float("{:.2f}".format(m[5]))))
    average = statistics.mean(m) 
    std_dev = statistics.stdev(m)
...
def main(argv):
    ...
    file = open("summary_of_results_pasta.txt", "w") 
